...
**Grafana Dashboards**  
To visualize YDB cluster metrics, import the preconfigured Grafana dashboards using the following templates: [🔍 YDB Grafana Templates](https://github.com/ydb-platform/ydb/tree/main/ydb/deploy/helm/ydb-prometheus/dashboards)  

**Prometheus Metrics Collection**
Metrics collection requires Prometheus with YDB-specific configuration files generated by Ansible playbook. The `generate_promconf` playbook generates complete Prometheus configurations including target definitions. You have two deployment options:  

1. **Standalone Deployment**  
   Use the generated `prometheus_ydb.yml` as your main Prometheus configuration file. This contains all necessary scrape jobs and target definitions for YDB monitoring.  

2. **Existing Prometheus Integration**  
   Merge relevant sections from these generated files into your existing Prometheus setup.

**Usage**  
```bash
ansible-playbook ydb_platform.ydb.generate_promconf # Generate configs
cat promconf/ydb_database.yml  # Verify node endpoints
```
