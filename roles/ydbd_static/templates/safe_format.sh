...
set +e
LD_LIBRARY_PATH={{ ydb_dir }}/lib {{ ydb_dir }}/bin/ydbd admin bs disk info -k 8229298086344098676 ${disk_part} >/dev/null 2>&1
CHECK_RESULT=$?
set -e
if [ $CHECK_RESULT -eq 0 ]; then
    ...
    if [ "${OVAL0}" == "${OVAL1}" ]; then
        ...
        LD_LIBRARY_PATH={{ ydb_dir }}/lib {{ ydb_dir }}/bin/ydbd admin bs disk obliterate ${disk_part}
    fi
fi
